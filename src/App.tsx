  ...
  return (
    <div className="app">
                ...
                <div className="phrasebook-category-grid">
                  {(["Greetings", "Travel", "Shopping", "Daily life"] as const).map((title) => {
                    ...
                    return (
                      ...
                      >
                        <span>{title}</span>
                        <small>{items.length} {items.length === 1 ? "phrase" : "phrases"}</small>
      ...
      <nav className="book-bottom-bar" aria-label="Primary navigation">
        ...
        <button type="button" className={page === "dictionary" ? "active" : ""} onClick={() => goTo("dictionary")} aria-label="Dictionary" aria-current={page === "dictionary" ? "page" : undefined}>
          <BookOpen size={18} />
