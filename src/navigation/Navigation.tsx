...
import { ArrowLeft, LocateFixed, MapPin, Navigation as NavigationIcon, RefreshCw, Route, Search, X } from "lucide-react";
...
export default function Navigation({ onBack }: NavigationProps) {
  ...
  const destinationResults = nodes
    .filter((node) => {
  ...
  return (
    <section className="navigation-page" aria-label="Offline navigation">
      ...
      <div className="navigation-destination-search">
        <div className="navigation-search-box">
          <Search size={17} />
          <input
            value={destinationSearch}
            onChange={(event) => setDestinationSearch(event.target.value)}
            placeholder="Search destination, e.g. CBU or Kitwe"
            ...
            {destinationResults.length > 0 ? (
              destinationResults.map((node) => (
                <button
                  key={node.id}
                  type="button"
                  className={destination?.id === node.id ? "navigation-search-result selected" : "navigation-search-result"}
                  onClick={() => {
                    chooseDestination(node);
                    setDestinationSearch(node.name);
                  }}
                >
                  <span className="place-icon"><MapPin size={16} /></span>
                  <span className="place-text">
                    <strong>{node.name}</strong>
                    <small>{node.type ?? "Mapped destination"}</small>
                  </span>
                </button>
              ))
            ) : (
              <div className="navigation-search-empty">No mapped destination found yet.</div>
            )}
          </div>
        )}
      </div>
